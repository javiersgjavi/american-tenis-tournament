...
def run_small_tournament_optimization(quick: bool = False):
    ...
    # Test parameters
    optimizer.test_population_sizes(base_config, sizes=[25, 50, 75, 100], n_trials=n_trials)
    optimizer.test_generation_counts(base_config, counts=[50, 100, 150], n_trials=n_trials)
...
def run_large_tournament_optimization(quick: bool = False):
    ...
    # Test parameters
    optimizer.test_population_sizes(base_config, sizes=[100, 150, 200, 250], n_trials=n_trials)
    optimizer.test_generation_counts(base_config, counts=[200, 300, 500], n_trials=n_trials)
    optimizer.test_mutation_rates(base_config, rates=[0.1, 0.15, 0.2], n_trials=n_trials)
    optimizer.test_crossover_rates(base_config, rates=[0.6, 0.7, 0.8], n_trials=n_trials)
