...
def print_heuristic_details(calendar: Calendar) -> None:
    ...
    if team_counts:
        ...
        repeated_teams = [(pair, count) for pair, count in team_counts.items() if count > 1]
    ...
    if opponent_counts:
        ...
        repeated_opponents = [(pair, count) for pair, count in opponent_counts.items() if count > 1]
    ...
    if flexibility_percent >= 50:
        print(f"    ✓ EXCELLENT - Very flexible ({flexibility_percent:.1f}% are cut points)")
    elif flexibility_percent >= 30:
        print(f"    ✓ GOOD - Quite flexible ({flexibility_percent:.1f}% are cut points)")
    elif flexibility_percent >= 15:
        print(f"    ⚠ ACCEPTABLE - Moderately flexible ({flexibility_percent:.1f}% are cut points)")
    else:
        print(f"    ⚠ LOW - Little flexibility ({flexibility_percent:.1f}% are cut points)")
