...
class HyperparameterOptimizer:
    ...
    def run_single_trial(
        self, config: HyperparameterConfig, verbose: bool = False
    ) -> OptimizationResult:
        """
        ...
        if len(acceptable_cuts) >= 2:
            gaps = [
                acceptable_cuts[i + 1] - acceptable_cuts[i]
    ...
    def run_multiple_trials(
        self, config: HyperparameterConfig, n_trials: int = 5, verbose: bool = False
    ...
    def test_population_sizes(
        ...
        for size in size_iterator:
            ...
            config = HyperparameterConfig(
                **{**base_config.to_dict(), "population_size": size}
            )
    ...
    def test_generation_counts(
        ...
        for count in count_iterator:
            ...
            config = HyperparameterConfig(
                **{**base_config.to_dict(), "generations": count}
            )
            trial_results = self.run_multiple_trials(config, n_trials, verbose=False)
            results.extend(trial_results)
    ...
    def test_mutation_rates(
        ...
        for rate in rate_iterator:
            ...
            config = HyperparameterConfig(
                **{**base_config.to_dict(), "mutation_rate": rate}
            )
            trial_results = self.run_multiple_trials(config, n_trials, verbose=False)
            results.extend(trial_results)
    ...
    def test_crossover_rates(
        ...
        for rate in rate_iterator:
            ...
            config = HyperparameterConfig(
                **{**base_config.to_dict(), "crossover_rate": rate}
            )
            trial_results = self.run_multiple_trials(config, n_trials, verbose=False)
            results.extend(trial_results)
    ...
    def test_elitism_sizes(
        ...
        for size in size_iterator:
            ...
            config = HyperparameterConfig(
                **{**base_config.to_dict(), "elitism_size": size}
            )
            trial_results = self.run_multiple_trials(config, n_trials, verbose=False)
            results.extend(trial_results)
    ...
    def _print_summary(self, results: list[OptimizationResult], title: str) -> None:
        ...
        print(f"\n{title} Summary:")
        print(
            f"  Fitness:    {np.mean(fitness_values):8.2f} ± {np.std(fitness_values):6.2f}"
        )
    ...
    def analyze_results(self) -> dict[str, Any]:
        ...
        successful = sum(
            1 for r in self.results if r.quality_level in ["EXCELLENT", "GOOD"]
        )
    ...
    def print_analysis(self) -> None:
        ...
        print(
            f"  Range:   [{analysis['fitness']['min']:.2f}, {analysis['fitness']['max']:.2f}]"
        )
        ...
        print(
            f"  Range:   [{analysis['execution_time']['min']:.2f}s, {analysis['execution_time']['max']:.2f}s]"
        )
        ...
        print(
            f"  Range:   [{analysis['cut_points']['min']}, {analysis['cut_points']['max']}]"
        )
        ...
        print(
            f"  Range:   [{analysis['balance']['min']}, {analysis['balance']['max']}]"
        )
