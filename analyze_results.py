...
def analyze_results(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for config_key, config_results in config_groups.items():
        ...
        config_stats[config_key] = {
            ...
            "avg_time": (
                statistics.mean(valid_times) if valid_times else statistics.mean(times)
            ),
    ...
    return {"by_config": config_stats, "all_results": results}
...
def find_best_configs(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    for config_key, stats in config_stats.items():
        ...
        # Fastest (valid time)
        valid_times = [
            r["execution_time"] for r in stats["results"] if r["execution_time"] < 1000
        ]
        if valid_times:
...
def print_analysis(results_file: Path):
    ...
    if best_configs["highest_fitness"]:
        ...
        print(
            f"Average fitness: {stats['avg_fitness']:.2f} ± {stats['std_fitness']:.2f}"
        )
        print(f"Maximum fitness: {stats['max_fitness']:.2f}")
        print(
            f"Average cut points: {stats['avg_cut_points']:.1f} (max: {stats['max_cut_points']})"
        )
    ...
    for pop_size in sorted(pop_groups.keys()):
        ...
        valid_times = [
            r["execution_time"] for r in group_results if r["execution_time"] < 1000
        ]
        cut_points = [r["acceptable_cuts_count"] for r in group_results]

        print(
            f"  Population {pop_size:3d}: Fitness={statistics.mean(fitnesses):7.1f}, "
    ...
    for mut_rate in sorted(mut_groups.keys()):
        ...
        print(
            f"  Mutation {mut_rate:.2f}: Fitness={statistics.mean(fitnesses):7.1f}, "
            f"Cut points={statistics.mean(cut_points):4.1f}, "
            f"Time={statistics.mean(valid_times) if valid_times else 0:.1f}s "
            f"(n={len(group_results)})"
        )
    ...
    for cross_rate in sorted(cross_groups.keys()):
        ...
        print(
            f"  Crossover {cross_rate:.2f}: Fitness={statistics.mean(fitnesses):7.1f}, "
            f"Cut points={statistics.mean(cut_points):4.1f}, "
            f"Time={statistics.mean(valid_times) if valid_times else 0:.1f}s "
            f"(n={len(group_results)})"
        )
    ...
    for elit_size in sorted(elit_groups.keys()):
        ...
        print(
            f"  Elitism {elit_size}: Fitness={statistics.mean(fitnesses):7.1f}, "
            f"Cut points={statistics.mean(cut_points):4.1f}, "
            f"Time={statistics.mean(valid_times) if valid_times else 0:.1f}s "
            f"(n={len(group_results)})"
        )
