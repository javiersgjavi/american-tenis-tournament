...
def main():
    ...
    POPULATION_SIZE = (
        200  # Size of the population (optimal: 100-150 for medium tournaments)
    )
    GENERATIONS = (
    ...
    CROSSOVER_RATE = (
        0.8  # Probability of crossover (optimal: 0.8 for good recombination)
    )
    ...
    WEIGHT_EARLY_CUT = (
        75.0  # Very High - strongly incentivize early cut points (increased from 50.0)
    )
    ...
    print(
        f"  • Early stopping patience: {EARLY_STOPPING_PATIENCE if EARLY_STOPPING_PATIENCE else 'disabled'}"
    )
    ...
    print_results(
        calendar=best_calendar, title="AMERICAN PADEL TOURNAMENT - OPTIMIZED CALENDAR"
    )
    ...
    if len(ga.best_fitness_history) > 0:
        ...
        print(
            f"  Total improvement: {improvement:.2f} ({(improvement/abs(initial_fitness)*100):.1f}%)"
        )

        # Show some milestones
        milestones = [
            0,
            len(ga.best_fitness_history) // 4,
            len(ga.best_fitness_history) // 2,
            3 * len(ga.best_fitness_history) // 4,
            len(ga.best_fitness_history) - 1,
        ]
    ...
    exported_files = export_all_outputs(
        best_calendar, output_dir="outputs", base_filename="tournament"
    )
